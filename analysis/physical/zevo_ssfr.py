...
properties = ['SFR_inst_30', 'SFR/SFR_10', 'SFR/SFR_100']

labels = {}

labels['SFR_inst_30'] = r'$\rm log_{10}[sSFR_{i}\;/\;yr^{-1}]$'
labels['SFR/SFR_10'] = r'$\rm log_{10}[sSFR_{10}\;/\;yr^{-1}]$'
labels['SFR/SFR_100'] = r'$\rm log_{10}[sSFR_{100}\;/\;yr^{-1}]$'
...
for property in properties:
    ...
    if property in ['SFR_inst_30', 'SFR/SFR_10', 'SFR/SFR_100', 'Mstar_30']:
        ...
        Y2 = fl.load_dataset('Mstar_30', arr_type='Galaxy')
    ...
    for i, tag in enumerate(fl.tags): # loop over snapshots (redshifts)
        ...
        ws, x, y, q = np.array([]), np.array([]), np.array([]), np.array([])
        for ii in range(len(halo)):
            ws = np.append(ws, np.ones(np.shape(H[halo[ii]][tag]))*weights[ii])
            x = np.append(x, np.log10(H[halo[ii]][tag]))

            if property in ['SFR_inst_30', 'SFR/SFR_10', 'SFR/SFR_100', 'Mstar_30']:
                y = np.append(y, np.log10(Y[halo[ii]][tag]/(Y2[halo[ii]][tag]*1e10)))
    ...
    fig.savefig(f'figures/paper/zevo_s{property.replace("/","_")}.pdf', bbox_inches='tight')
    fig.clf()
