...
filters = ['Euclid/NISP/Y', 'Euclid/NISP/J', 'Euclid/NISP/H', 'Spitzer/IRAC/ch1', 'Spitzer/IRAC/ch2']
...
for j, phot_type in enumerate(phot_types):
    ...
    for i, tag in enumerate(fl.tags): # loop over snapshots (redshifts)
        ...
        for i, filter in enumerate(filters):
            c1.extend((np.ones(len(l[filter]))*i).flatten)
            c2.extend(np.array([l[filter] for filter in filters]).flatten)
        ...
        #ax.scatter(c1, c2, s=2, alpha=0.25)
